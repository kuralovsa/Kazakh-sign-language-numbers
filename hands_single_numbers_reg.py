    ...
    try:
        ...
        for handLms in result.multi_hand_landmarks:
            for id, lm in enumerate(handLms.landmark):
                h, w, c = img_flip.shape
                cx, cy = int(lm.x * w), int(lm.y * h)
                cv2.circle(hand_img, (cx, cy), 15, (255, 0, 255), cv2.FILLED)
            mpDraw.draw_landmarks(hand_img, handLms, mp_hands.HAND_CONNECTIONS)
...
# load model
with open('modeltest.pkl', 'rb') as f:
    svm = pickle.load(f)
